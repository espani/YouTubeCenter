...
function request(wrappedContentWin, chromeWin, details) {
...
function addEventListener(wrappedContentWin, req, event, details){
  ...
  req.addEventListener(event, function(e) {
    var responseState = {
      __exposedProps__: {
        context: "r",
        finalUrl: "r",
    ...
    switch (event) {
      case "progress":
        responseState.lengthComputable = e.lengthComputable;
        responseState.loaded = e.loaded;
        responseState.total = e.total;
        break;
      case "error":
        break;
      default:
